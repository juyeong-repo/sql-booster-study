...
Q. 그럼 옵티마이저가 알아서 인덱스도 like면 2조건으로 넣던가 이런식으로 하는것일까?

데이터베이스 옵티마이저는 쿼리를 분석하여 가장 효율적인 실행 계획을 자동으로 선택한다. 

이 과정에서 어떤 인덱스를 사용할지, 어떤 순서로 테이블을 조인할지 등을 결정한다.
...
→ DB는 n byte 가져와! 가 아니라 n **페이지** 가져와! 라고 OS에 요청한다
...
> 핵심 : ②번 히트가 많을수록 물리 I/O가 줄고, ③·④ 빈도가 떨어지면서 버퍼 사용량 감소 → 속도가 올라감!!!
> 
...
→ 버퍼 풀은 *모든 세션이 공유*, 이 영역들은 *세션/쿼리별 독립*이므로 합산 메모리 추정이 필요함
...
```
+----+-------------------------------+------------------+---------+------------+----------+
| ID | OPERATION                     | NAME             | ROWS    | A-TIME     | BUFFERS  |
+----+-------------------------------+------------------+---------+------------+----------+
|  0 | SELECT STATEMENT              |                  |       1 | 00:00:04.463 |          |
|  1 | HASH GROUP BY                 |                  |       1 | 00:00:04.463 |          |
|  2 | TABLE ACCESS BY INDEX ROWID   | T_ORD_BIG        | 1300000 | 00:00:04.323 |          |
|  3 | INDEX RANGE SCAN              | X_T_ORD_BIG_4    | 1300000 | 00:00:00.091 |          |
+----+-------------------------------+------------------+---------+------------+----------+
...
| ID | OPERATION          | NAME              | ROWS   | A-TIME     | BUFFERS               |
+----+--------------------+-------------------+--------+------------+-----------------------+
|  0 | SELECT STATEMENT   |                   |      1 | 00:00:00.031 |                       |
|  1 | HASH GROUP BY      |                   |      1 | 00:00:00.031 |                       |
|  2 | INDEX RANGE SCAN   | X_T_ORD_BIG_4     | 1300000| 00:00:00.031 |                       |
+----+--------------------+-------------------+--------+------------+-----------------------+
...
+------------------+--------------+-----------+---------------+
| T_ORD_BIG        | TABLE        | 2,048     | 70            |
| X_T_ORD_BIG_1    | INDEX        |   672     | 23            |
| X_T_ORD_BIG_2    | INDEX        |   504     | 17            |
| X_T_ORD_BIG_3    | INDEX        |   974     | 34            |
| X_T_ORD_BIG_4    | INDEX        | 1,152     | 40            |
+------------------+--------------+-----------+---------------+
```
...
```json
   ...
   +-------------+ +-------------+ +-------------+ +-------------+
   |    X_1      | |    X_2      | |    X_3      | |    X_4      |
   |   672MB     | |   504MB     | |   974MB     | |  1,152MB    |
   | 23 Byte/row | | 17 Byte/row | | 34 Byte/row | | 40 Byte/row |
   +-------------+ +-------------+ +-------------+ +-------------+
...
> 예: CUS_ID(J), ORD_DT(O) → CUS_ID는 조인, ORD_DT는 정렬 대상
> 
